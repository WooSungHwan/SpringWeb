		            		...
		            		<sec:authorize access="isAuthenticated()">
				            	<c:if test="${pinfo.username eq board.writer}">
						        	<button data-oper="modify" class="btn btn-default" onclick=
										'location.href="/board/modify?bno=${board.bno}"'>수정하기</button>
						        </c:if>
					        </sec:authorize>
	...
	$(document).ready(function() {
	    ...
	    modalModBtn.on("click",function(e){
	    	...
	    	var reply = {
	    		...
	    		replyer : originalReplyer //댓글 주인 전달.
	    	};
	    	
	    	if(!replyer){ //현재 로그인한 사용자
	    		alert("로그인 후 수정이 가능합니다.");
	    		modal.modal("hide");
	    		return;
	    	}
	    	
	    	console.log("원래 댓글 등록자 : "+originalReplyer);
		...
		/* replyService.remove(52, function(count){
			console.log(count);
		...
		replyService.get(82, function(data) {
			console.log(data);
		});
...
<script>
	$(document).ready(function(){
		...
		(function(){
			...
			//이미지 바깥쪽 클릭하면 작아지는 애니메이션 효과 내면서 사라짐.
			...
			//이미지 보이게함.
			function showImage(fileCallPath){ //전체 이미지 경로. 2019부터
